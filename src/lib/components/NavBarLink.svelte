...
{#if type == 'desktop'}
	...
	>
		<span
			class={`absolute inset-0 h-full rounded bg-yellow-300 transition-transform duration-300 ease-in-out group-hover:translate-y-0 ${isActive ? 'translate-y-6' : 'translate-y-8'}`}
		></span>
...
{:else}
	...
	>
		<span
			class={`absolute inset-0 h-full rounded bg-yellow-300 transition-transform duration-300 ease-in-out group-hover:translate-y-0 ${isActive ? 'translate-y-6' : 'translate-y-20'}`}
		></span>
		<span class="relative">{label}</span>
	</PrismicLink>
{/if}
