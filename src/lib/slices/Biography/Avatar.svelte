...
<div class={clsx('relative h-full w-full ', className)} bind:this={component}>
	<div class="avatar aspect-square rounded-3xl overfow-hidden border-2 border-slate-700 opacity-0">
		<PrismicImage
			field={image}
			class="avatar-image h-full w-full object-fill rounded-3xl"
			imgixParams={{ q: 100 }}
		/>
		<div
			class="highlight absolute inset-0 w-full scale-102 bg-gradient-to-tl from-transparent via-white to-transparent opacity-0 rounded-3xl"
		></div>
	</div>
</div>
